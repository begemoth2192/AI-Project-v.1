...
function App() {
  ...
  useEffect(()=>{
    (async()=>{
      try {
        const raw = localStorage.getItem("vos_v2");
        if(raw){ const d=JSON.parse(raw); setUser(d.user); setProfile(d.profile); setPoints(d.points||0); setStreak(d.streak||0); setSessions(d.sessions||[]); if(d.profile) setPage("main"); }
      } catch(e){}
    })();
  },[]);
  ...
  const save = useCallback(async(overrides={})=>{
    try{
      ...
      localStorage.setItem("vos_v2",JSON.stringify(d));
    }catch(e){}
  ...
  const handleLogout = async()=>{
    try{localStorage.removeItem("vos_v2")}catch(e){}
